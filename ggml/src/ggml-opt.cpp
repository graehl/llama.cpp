...
struct ggml_opt_context {
    ...
    // has the 5 adamw parms plus 3 derived values for cuda efficiency (total 8)
...
static void ggml_opt_build(ggml_opt_context_t opt_ctx) {
    ...
    const bool moment = opt_ctx->build_type_alloc == GGML_OPT_BUILD_TYPE_OPT &&
                        (need_moment(opt_ctx->opt_pars()) || has_moment(opt_ctx));
...
void ggml_opt_eval(ggml_opt_context_t opt_ctx, ggml_opt_result_t result) {
    ...
    if (opt_ctx->allocated_graph == opt_ctx->gb_opt) {
        ...
        GGML_ASSERT(opt_pars.common.alpha > 0.0f);
        GGML_ASSERT(opt_pars.adamw.beta1 >= 0.0f);
        GGML_ASSERT(opt_pars.adamw.beta1 <= 1.0f);
        GGML_ASSERT(opt_pars.adamw.beta2 >= 0.0f);
        GGML_ASSERT(opt_pars.adamw.beta2 <= 1.0f);
        GGML_ASSERT(opt_pars.adamw.eps   >= 0.0f);
        GGML_ASSERT(opt_pars.common.wd >= 0.0f);
        GGML_ASSERT(opt_pars.common.wd <= 1.0f);

        // beta1, beta2 after applying warmup
        const float beta1h = 1.0f/(1.0f - powf(opt_pars.adamw.beta1, opt_ctx->iter));
        const float beta2h = 1.0f/(1.0f - powf(opt_pars.adamw.beta2, opt_ctx->iter));
        const float keep           = 1.0f - opt_pars.common.alpha * opt_pars.common.wd;
        float * adamw_par_data = ggml_get_data_f32(opt_ctx->adamw_params);
        adamw_par_data[0]          = opt_pars.common.alpha;
        adamw_par_data[1] = opt_pars.adamw.beta1;
        adamw_par_data[2] = opt_pars.adamw.beta2;
        adamw_par_data[3] = opt_pars.adamw.eps;
        adamw_par_data[4]          = opt_pars.common.wd;
        adamw_par_data[5] = beta1h;
        adamw_par_data[6] = beta2h;
        adamw_par_data[7]          = keep;
    }
