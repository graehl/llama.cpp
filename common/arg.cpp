...
gpt_params_context gpt_params_parser_init(gpt_params & params, llama_example ex, void(*print_usage)(int, char **)) {
    ...
    add_opt(llama_arg(
        {"--penalize-nl"},
        format("stop generating (in non-interactive/assistant context) after newline token (default: %s)", params.sparams.stop_nl ? "true" : "false"),
        [](gpt_params & params) {
            params.sparams.stop_nl = true;
        }
    ).set_sparam());
    add_opt(llama_arg(
        {"--temp"}, "N",
